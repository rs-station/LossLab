...
from LossLab.utils.alignment import (
    cra_exclude_mask,
    extract_allatoms,
    position_alignment,
    weighted_rmsd,
...
# Suppress unused-import warning: np is used implicitly via rk_utils
_np = np
...
class CryoEMLLGLoss(BaseLoss):
    ...
    def __init__(
        ...
        self.best_pos = self.reference_pos.clone()
        self._init_bfactors()
        ...
        if self.fixed_b_factor is not None:
            ...
            logger.info(
                "Using fixed B-factor = %.1f for all atoms",
                self.fixed_b_factor,
            )
    ...
    def _load_pdb_no_h(cif_path: str) -> PDBParser:
        """Load CIF/PDB, strip hydrogens and ligands/waters."""
    ...
    def _update_bfactors_rscc(self, aligned_xyz: torch.Tensor) -> None:
        """Recompute RSCC B-factors — updates both SFCs."""
    ...
    def compute_from_rollout(
        ...
        # 4. Optional SFC scale on RBR SFC
        if self.sfc_scale:
            self.sfc_rbr.calc_fprotein()
            self.sfc_rbr.get_scales_adam(
                lr=0.01,
                n_steps=10,
                sub_ratio=0.7,
                initialize=False,
        ...
        # 5. Rigid-body refinement (RBR) via quaternion LBFGS
        ...
        # 8. B-factor weighted L2 penalty
        l2_loss = torch.sum(
        ...
        try:
            ...
            rmsd_aligned = float(
                weighted_rmsd(aligned_xyz.detach(), ref, w=w, mask=rmsd_mask)
            )
            rmsd_rbr = float(
                weighted_rmsd(optimized_xyz.detach(), ref, w=w, mask=rmsd_mask)
            )
        except Exception:
        ...
        md: dict[str, Any] = {
            ...
            "optimized_xyz": optimized_xyz.detach().clone(),
        }
    ...
    def update_best_pos(self, new_best_xyz: torch.Tensor) -> None:
        """Update Kabsch alignment target to the best coordinates."""
        self.best_pos = new_best_xyz.detach().clone().to(self.device)
